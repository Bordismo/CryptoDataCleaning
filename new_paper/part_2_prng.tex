...
\usepackage{amsthm,amsmath,amsfonts,amssymb,color, enumerate,tikz, hyperref, algpseudocode, amscd, url, graphicx, floatrow, subfig, float}
\usepackage[all]{xy}
...
% \author[C. Lee]{Choong-Soo Lee}
% \address{Insert Info here  }
...
\keywords{Cryptography, Pseudo-random number generator}
...
follows immediately from Theorem \ref{N simplex} and the tests found in sections \S\ref{results} and \S\ref{appendix}. %\textcolor{red}{add section for the results showing that the test suite does not see these bit sequences}.
...
For the first experiment we fix the h-vector length at 3751, with a pattern of all 1's, and vary the number of coning operations from 0 to 99 times. (see Figure~\ref{fig:conings})  %    '-p 3751 -f -c 0 -b' through '-p 3751 -f -c 99 -b' 
For the second experiment we vary the h-vector length from 3750 to 3849, with a pattern of all 1's, and we do not apply the coning operation.~\ref{fig:lengths}   %  '-p 3750 -f -b' through '-p 3849 -f -b'
For the third experiment we fix the h-vector length at 3750, vary non-end elements in the pattern from 1 to 100, and do no apply the coning operation.~\ref{fig:patterns}  %   '-p 3750 -i 1 -f -b' through '-p 3750 -i 100 -f -b'
...
% RESULTS CHARTS HERE
% add link to repo for varying conings
...
\begin{figure}[h!]
\centering
\caption{100 trendlines for NIST tests p-values on vectors of lengths between 3750 and 3849 with a pattern of all 1's}
\includegraphics[width=0.8\linewidth, height=0.5\linewidth]{./charts/varying-lengths-v2.png}
\label{fig:lengths}
\end{figure}

\begin{figure}[h!]
\centering
\caption{100 trendlines for NIST tests p-values on vectors of length 3750 with non-end elements from 1 to 100.}
\includegraphics[width=0.8\linewidth, height=0.5\linewidth]{./charts/varying-patterns-v2.png}
\label{fig:patterns}
\end{figure}


% \centering
% \caption{Trendlines for exeriments with varying conings from 0-99 times (left), varying h-vector lengths from 3750-3849 (mid) and varying patterns for h-vector length of 3750 and non-end elements from 1-100 (right).}%
% \label{fig:images}
% \subfloat[left]{%
%      \includegraphics[width=0.18\linewidth]{./charts/varying-conings-v2.png}}
%      \label{fig:conings}
% \qquad
% \subfloat[mid]{%
%      \includegraphics[width=0.18\linewidth]{./charts/varying-lengths-v2.png}}
%      \label{fig:lengths}
% \qquad
% \subfloat[right]{%
...
%\begin{floatrow}[3]%
%\ffigbox[\FBwidth]{\caption{varying conings}\label{fig:conings}}{\includegraphics[width=.2\textwidth]{./charts/varying-conings-v2.png}}
%\ffigbox[\FBwidth]{\caption{varying lengths}\label{fig:lengths}}{\includegraphics[width=.2\textwidth]{./charts/varying-lengths-v2.png}}
%\ffigbox[\FBwidth]{\caption{varying patterns}\label{fig:patterns}}{\includegraphics[width=.2\textwidth]{./charts/varying-patterns-v2.png}}
%\end{floatrow}
%\end{figure}
...
\bibitem[NIST]{NIST} A. Runkhin., J. Soto., J. Nechvatal., M. Smid., E., Baker, S. Leigh., M. Levenson., M. Vangel.,
...
\bibitem[NLKB]{NLKB} S. Nobari., X. Lu., P. Karras., S. Bressan.,
...
\bibitem[NSC]{NSC} National Science and Technology Council., Chairs:  Holdren, J., Shannon, G., Co-chairs: Kurose., J. Marzullo, K.,
